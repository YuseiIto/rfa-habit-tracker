...
const putWorkoutWithQuantity = async (env: Env, date: Date, quantity: number) => {
	const client = api(aspida())

	await client.v1.users._username(env.pixelaUsername).graphs._graphID(env.pixelaGraphId)._yyyyMMdd(format(date, 'yyyyMMdd')).$put({
		headers: {
...
export const endWorkout = async (env: Env): Promise<LambdaFunctionURLResult> => {
	console.log("EndWorkout called")

	const client = api(aspida())

	const res = await client.v1.users._username(env.pixelaUsername).graphs._graphID(env.pixelaGraphId).latest.get({
		headers: {
			'X-USER-TOKEN': env.pixelaToken
		}
	}).catch((e) => {
	...
	if (res.status !== 200) {
		...
		console.error("No last pixel found. Maybe start pixel is not posted. Fixing...")
		...
		if (payload.type === 'start') {
			...
			if (diff_ms > 24 * 60 * 60 * 1000) {
				console.warn("The last pixel is 'start', but it's older than 24 hours. Fixing...")
			...
			if (diff_ms < 60 * 1000) {
				console.warn("EndWorkout posted within 60 seconds. Maybe it's called twice. Ignoring")
			} else {
				console.warn("EndWorkout requested, but the latest pixel is 'end'. So the quantity will be set to default...")
				await putDefaultWorkout(env, new Date())
			}
		} else {
